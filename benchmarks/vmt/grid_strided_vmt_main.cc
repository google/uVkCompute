...
static const char kBenchmarkName[] = "grid_strided_vmt";
...
#define SHADER_I8(B0, N0, K0, X, Y)                                            \
  ShaderCode {                                                                 \
    "Tile[" #N0 "x" #K0 "]",                                                   \
        vmt_i8::B0_##B0##_N0_##N0##_K0_##K0##_WG_X_##X##_WG_Y_##Y, B0, N0, K0, \
        X, Y, DataType::i8, DataType::i32                                      \
  }

#define WORKGROUP_TILE_N_I8(B0, X, Y, N0)                     \
  SHADER_I8(B0, N0, 4, X, Y), SHADER_I8(B0, N0, 4, X, Y),     \
      SHADER_I8(B0, N0, 8, X, Y), SHADER_I8(B0, N0, 8, X, Y), \
      SHADER_I8(B0, N0, 16, X, Y), SHADER_I8(B0, N0, 16, X, Y)

#if defined(UVKC_RDNA3)

namespace vmt_i8 {
#include "grid_strided_vmt_i8_shader_rdna3_spirv_permutation.inc"
}

static ShaderCode kShaderCodeCases[] = {
    WORKGROUP_TILE_N_I8(256, 64, 1, 1), WORKGROUP_TILE_N_I8(256, 64, 1, 2),
    WORKGROUP_TILE_N_I8(256, 64, 1, 4), WORKGROUP_TILE_N_I8(256, 64, 2, 2),
    WORKGROUP_TILE_N_I8(256, 64, 2, 4), WORKGROUP_TILE_N_I8(256, 64, 4, 4),
    WORKGROUP_TILE_N_I8(128, 64, 1, 1), WORKGROUP_TILE_N_I8(128, 64, 1, 2),
    WORKGROUP_TILE_N_I8(128, 64, 1, 4), WORKGROUP_TILE_N_I8(128, 64, 2, 2),
    WORKGROUP_TILE_N_I8(128, 64, 2, 4), WORKGROUP_TILE_N_I8(128, 64, 4, 4),
};
...
static void Vmt(::benchmark::State &state, ::uvkc::vulkan::Device *device,
  ...
  // Each workgroup has a fixed number of workgroups.
  dispatch_cmdbuf->Dispatch(shader.B0, 1, 1);
